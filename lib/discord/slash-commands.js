// /lib/discord/slash-commands.js
// Discord slash commands for Grape Verification Registry (Next.js API handler friendly)

const bs58 = require("bs58");
const { Connection, Keypair, PublicKey, Transaction } = require("@solana/web3.js");

let gvrPromise = null;
async function getGvr() {
  if (!gvrPromise) gvrPromise = import("@grapenpm/grape-verification-registry");
  return gvrPromise;
}

/* -----------------------------
   Discord response helpers
------------------------------ */

function json(res, code, body) {
  res.statusCode = code;
  res.setHeader("Content-Type", "application/json; charset=utf-8");
  res.setHeader("Cache-Control", "no-store");
  res.end(JSON.stringify(body));
}

function reply(content, opts = {}) {
  // Discord Interaction response format
  return {
    type: 4, // CHANNEL_MESSAGE_WITH_SOURCE
    data: {
      content,
      flags: opts.ephemeral ? 64 : 0,
    },
  };
}

function replyWithLink(content, label, url, opts = {}) {
  return {
    type: 4,
    data: {
      content,
      flags: opts.ephemeral ? 64 : 0,
      components: [
        {
          type: 1,
          components: [
            {
              type: 2,
              style: 5,
              label,
              url,
            },
          ],
        },
      ],
    },
  };
}

function replyWithEmbed(title, description, opts = {}) {
  return {
    type: 4,
    data: {
      flags: opts.ephemeral ? 64 : 0,
      embeds: [
        {
          title,
          description,
        },
      ],
    },
  };
}

/* -----------------------------
   Env / Config
------------------------------ */

function mustEnv(name) {
  const v = process.env[name];
  if (!v) throw new Error(`Missing ${name}`);
  return v;
}

function rpcUrl() {
  return (
    process.env.NEXT_PUBLIC_SOLANA_RPC ||
    process.env.NEXT_PUBLIC_SOLANA_RPC_URL ||
    process.env.NEXT_PUBLIC_RPC_SOLANA_MAINNET ||
    process.env.REACT_APP_RPC_ENDPOINT ||
    process.env.SOLANA_RPC_URL ||
    "https://api.mainnet-beta.solana.com"
  );
}

function verificationUrl() {
  return process.env.VERIFICATION_URL || "https://verification.governance.so";
}

function defaultDaoPubkey() {
  return (
    process.env.GVR_DAO ||
    process.env.GVR_DEFAULT_DAO ||
    process.env.DAO_PUBKEY ||
    process.env.NEXT_PUBLIC_GVR_DAO ||
    ""
  );
}

function guildDaoMap() {
  const raw = process.env.GVR_GUILD_DAO_MAP;
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : null;
  } catch {
    return null;
  }
}

function daoForInteraction(interaction) {
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  const map = guildDaoMap();
  if (map && guildId && map[guildId]) return String(map[guildId]);
  return defaultDaoPubkey();
}

function defaultVerifiedRoleId() {
  return (
    process.env.GVR_VERIFIED_ROLE_ID ||
    process.env.VERIFIED_ROLE_ID ||
    ""
  );
}

function guildRoleMap() {
  const raw = process.env.GVR_GUILD_ROLE_MAP;
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : null;
  } catch {
    return null;
  }
}

function verifiedRoleForInteraction(interaction) {
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  const map = guildRoleMap();
  if (map && guildId && map[guildId]) return String(map[guildId]);
  return defaultVerifiedRoleId();
}

function envTrue(name, fallback = false) {
  const raw = process.env[name];
  if (raw == null || raw === "") return fallback;
  return ["1", "true", "yes", "y", "on"].includes(String(raw).toLowerCase());
}

function removeRoleOnUnverified() {
  return envTrue("GVR_REMOVE_ROLE_ON_UNVERIFIED", true);
}

let kvPromise = null;
async function getKv() {
  if (kvPromise) return kvPromise;
  kvPromise = (async () => {
    try {
      const mod = require("@vercel/kv");
      const client = mod?.kv;
      if (!client) return null;
      await client.ping?.();
      return client;
    } catch {
      return null;
    }
  })();
  return kvPromise;
}

function guildConfigKey(guildId) {
  return `gvr:config:${guildId}`;
}

async function getGuildConfig(guildId) {
  if (!guildId) return null;
  const kv = await getKv();
  if (!kv) return null;
  try {
    const cfg = await kv.get(guildConfigKey(guildId));
    return cfg && typeof cfg === "object" ? cfg : null;
  } catch {
    return null;
  }
}

async function setGuildConfig(guildId, patch) {
  const kv = await getKv();
  if (!kv) {
    throw new Error("KV is not configured. Add KV env vars or use GVR_DAO/GVR_GUILD_DAO_MAP.");
  }
  const key = guildConfigKey(guildId);
  const current = (await kv.get(key)) || {};
  const next = {
    ...current,
    ...patch,
    guildId,
    updatedAt: new Date().toISOString(),
  };
  await kv.set(key, next);
  return next;
}

async function resolvedDaoForInteraction(interaction) {
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  if (guildId) {
    const cfg = await getGuildConfig(guildId);
    if (cfg?.dao) return String(cfg.dao);
  }
  return daoForInteraction(interaction);
}

async function resolvedVerifiedRoleForInteraction(interaction) {
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  if (guildId) {
    const cfg = await getGuildConfig(guildId);
    if (cfg?.verifiedRoleId) return String(cfg.verifiedRoleId);
  }
  return verifiedRoleForInteraction(interaction);
}

const DISCORD_PERMISSION_ADMINISTRATOR = 1n << 3n;
const DISCORD_PERMISSION_MANAGE_GUILD = 1n << 5n;

function hasSetupPermission(interaction) {
  const permRaw = interaction?.member?.permissions;
  if (!permRaw) return false;
  try {
    const perms = BigInt(String(permRaw));
    return (
      (perms & DISCORD_PERMISSION_ADMINISTRATOR) !== 0n ||
      (perms & DISCORD_PERMISSION_MANAGE_GUILD) !== 0n
    );
  } catch {
    return false;
  }
}

function parsePubkeyOrThrow(value, fieldName) {
  const raw = String(value || "").trim();
  try {
    return new PublicKey(raw);
  } catch {
    const looksLikeDiscordId = /^\d{17,22}$/.test(raw);
    const hint = looksLikeDiscordId
      ? "It looks like a Discord ID. Use a Solana public key for your DAO."
      : "Expected a base58 Solana public key.";
    throw new Error(`Invalid ${fieldName}: ${raw}. ${hint}`);
  }
}

function parseKeypairFromEnv(name) {
  const raw = mustEnv(name).trim();

  // JSON array
  if (raw.startsWith("[") && raw.endsWith("]")) {
    const arr = JSON.parse(raw);
    return Keypair.fromSecretKey(Uint8Array.from(arr));
  }

  // base58
  try {
    const b = bs58.decode(raw);
    if (b?.length) return Keypair.fromSecretKey(new Uint8Array(b));
  } catch {}

  // base64
  try {
    const b = Buffer.from(raw, "base64");
    if (b?.length) return Keypair.fromSecretKey(new Uint8Array(b));
  } catch {}

  throw new Error(`${name} invalid format (use JSON array, base58, or base64)`);
}

/* -----------------------------
   Platform helpers
------------------------------ */

function platformEnum(platform, gvr) {
  const { VerificationPlatform } = gvr;
  switch ((platform || "").toLowerCase()) {
    case "discord":
      return VerificationPlatform.Discord;
    case "telegram":
      return VerificationPlatform.Telegram;
    case "twitter":
      return VerificationPlatform.Twitter;
    case "email":
      return VerificationPlatform.Email;
    default:
      return VerificationPlatform.Discord;
  }
}

function platformSeed(platform, gvr) {
  return platformEnum(platform, gvr);
}

function platformTag(platform, gvr) {
  const { TAG_DISCORD, TAG_TELEGRAM, TAG_TWITTER, TAG_EMAIL } = gvr;
  switch ((platform || "").toLowerCase()) {
    case "discord":
      return TAG_DISCORD;
    case "telegram":
      return TAG_TELEGRAM;
    case "twitter":
      return TAG_TWITTER;
    case "email":
      return TAG_EMAIL;
    default:
      return TAG_DISCORD;
  }
}

/* -----------------------------
   On-chain parsing (Space salt)
------------------------------ */

function parseSpaceSalt(spaceDataU8) {
  // matches your layout: disc(8) + version(1) + dao(32) + authority(32) + attestor(32) + is_frozen(1) + bump(1) + salt(32)
  const SALT_OFFSET = 8 + 1 + 32 + 32 + 32 + 1 + 1; // 107
  return spaceDataU8.slice(SALT_OFFSET, SALT_OFFSET + 32);
}

/* -----------------------------
   Discord command registration
------------------------------ */

const PLATFORM_CHOICES = [
  { name: "discord", value: "discord" },
  { name: "email", value: "email" },
  { name: "telegram", value: "telegram" },
  { name: "twitter", value: "twitter" },
];

const ADMIN_SUBCOMMANDS = [
  {
    type: 1,
    name: "status",
    description: "Show PDAs and on-chain status",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      {
        type: 3,
        name: "platform_user_id",
        description: "Platform user id (discord id, email id, etc)",
        required: true,
      },
      { type: 3, name: "wallet", description: "Wallet pubkey (optional)", required: false },
    ],
  },
  {
    type: 1,
    name: "space-init",
    description: "Initialize Space PDA for a DAO (admin/bot)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      { type: 3, name: "salt_hex", description: "32-byte hex (64 chars)", required: true },
    ],
  },
  {
    type: 1,
    name: "attest",
    description: "Attest identity (attestor signs tx)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      { type: 3, name: "platform_user_id", description: "Platform user id", required: true },
      { type: 4, name: "expires_seconds", description: "Expiry in seconds (0 = none)", required: false },
    ],
  },
  {
    type: 1,
    name: "revoke",
    description: "Revoke identity (attestor signs tx)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      { type: 3, name: "platform_user_id", description: "Platform user id", required: true },
    ],
  },
  {
    type: 1,
    name: "link",
    description: "Link a wallet to identity (attestor signs tx)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      { type: 3, name: "platform_user_id", description: "Platform user id", required: true },
      { type: 3, name: "wallet", description: "Wallet pubkey", required: true },
    ],
  },
  {
    type: 1,
    name: "link-self",
    description: "Link wallet where the wallet is a signer (requires wallet signature on tx)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      { type: 3, name: "platform_user_id", description: "Platform user id", required: true },
      { type: 3, name: "wallet", description: "Wallet pubkey", required: true },
    ],
  },
  {
    type: 1,
    name: "unlink",
    description: "Unlink a wallet from identity (attestor signs tx)",
    options: [
      { type: 3, name: "dao", description: "DAO pubkey", required: true },
      {
        type: 3,
        name: "platform",
        description: "discord|telegram|twitter|email",
        required: true,
        choices: PLATFORM_CHOICES,
      },
      { type: 3, name: "platform_user_id", description: "Platform user id", required: true },
      { type: 3, name: "wallet", description: "Wallet pubkey", required: true },
      {
        type: 3,
        name: "recipient",
        description: "Recipient for reclaimed lamports (defaults to bot fee payer)",
        required: false,
      },
    ],
  },
];

const COMMANDS = [
  {
    name: "gv",
    description: "Grape Verification",
    options: [
      {
        type: 1,
        name: "setup",
        description: "Admin: configure DAO and verified role for this server",
        options: [
          { type: 3, name: "dao", description: "DAO pubkey", required: true },
          {
            type: 8,
            name: "verified_role",
            description: "Role to assign when a user is verified (optional)",
            required: false,
          },
        ],
      },
      { type: 1, name: "verify", description: "Open web verification" },
      { type: 1, name: "check", description: "Check if you are verified" },
      { type: 1, name: "ping", description: "Health check" },
      { type: 2, name: "admin", description: "Admin operations", options: ADMIN_SUBCOMMANDS },
    ],
  },
];

async function discordApi(path, { method = "GET", body } = {}) {
  const token = mustEnv("DISCORD_BOT_TOKEN");
  const url = `https://discord.com/api/v10${path}`;
  const res = await fetch(url, {
    method,
    headers: {
      Authorization: `Bot ${token}`,
      "Content-Type": "application/json",
    },
    body: body ? JSON.stringify(body) : undefined,
  });
  const text = await res.text().catch(() => "");
  if (!res.ok) {
    throw new Error(`Discord API ${res.status}: ${text}`);
  }
  return text ? JSON.parse(text) : null;
}

async function registerCommands({ guildId } = {}) {
  const appId = process.env.DISCORD_APPLICATION_ID || process.env.DISCORD_APP_ID;
  if (!appId) throw new Error("Missing DISCORD_APPLICATION_ID or DISCORD_APP_ID");

  if (guildId) {
    return discordApi(`/applications/${appId}/guilds/${guildId}/commands`, {
      method: "PUT",
      body: COMMANDS,
    });
  }

  return discordApi(`/applications/${appId}/commands`, {
    method: "PUT",
    body: COMMANDS,
  });
}

async function addRoleToMember(guildId, userId, roleId) {
  return discordApi(`/guilds/${guildId}/members/${userId}/roles/${roleId}`, {
    method: "PUT",
  });
}

async function removeRoleFromMember(guildId, userId, roleId) {
  return discordApi(`/guilds/${guildId}/members/${userId}/roles/${roleId}`, {
    method: "DELETE",
  });
}

async function syncVerifiedRole(interaction, verified) {
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  const userId = interactionUserId(interaction);
  const roleId = await resolvedVerifiedRoleForInteraction(interaction);
  if (!guildId || !userId || !roleId) return "";

  try {
    if (verified) {
      await addRoleToMember(guildId, userId, roleId);
      return `\nRole sync: ✅ assigned <@&${roleId}>`;
    }
    if (removeRoleOnUnverified()) {
      await removeRoleFromMember(guildId, userId, roleId);
      return `\nRole sync: ℹ️ removed <@&${roleId}>`;
    }
    return `\nRole sync: ℹ️ kept unchanged (<@&${roleId}>)`;
  } catch (e) {
    const msg = String(e?.message || e);
    return `\nRole sync: ⚠️ failed (${msg})`;
  }
}

/* -----------------------------
   Solana tx helper
------------------------------ */

async function sendTx(connection, tx, signers) {
  tx.feePayer = signers[0].publicKey;

  const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash("confirmed");
  tx.recentBlockhash = blockhash;

  tx.sign(...signers);

  const sig = await connection.sendRawTransaction(tx.serialize(), {
    skipPreflight: false,
    maxRetries: 3,
  });

  await connection.confirmTransaction(
    { signature: sig, blockhash, lastValidBlockHeight },
    "confirmed"
  );

  return sig;
}

/* -----------------------------
   Command handlers
------------------------------ */

function optMap(interaction) {
  const opts = {};
  const first = interaction?.data?.options?.[0];
  if (!first) return { group: null, sub: null, opts };

  // Subcommand group: /gv admin <sub> ...
  if (first.type === 2) {
    const sub = first?.options?.[0];
    const subOpts = sub?.options || [];
    for (const o of subOpts) opts[o.name] = o.value;
    return { group: first.name, sub: sub?.name, opts };
  }

  const subOpts = first?.options || [];
  for (const o of subOpts) opts[o.name] = o.value;
  return { group: null, sub: first?.name, opts };
}

function interactionUserId(interaction) {
  return (
    interaction?.member?.user?.id ||
    interaction?.user?.id ||
    interaction?.member?.id ||
    null
  );
}

function buildVerificationUrl(interaction, dao) {
  const base = verificationUrl();
  const url = new URL(base);
  const userId = interactionUserId(interaction);
  const daoStr = dao ? String(dao).trim() : "";

  if (daoStr) {
    const basePath = url.pathname.replace(/\/+$/, "");
    const daoPath = `/dao/${encodeURIComponent(daoStr)}`;
    url.pathname = basePath ? `${basePath}${daoPath}` : daoPath;
  }

  url.searchParams.set("source", "discord");
  url.searchParams.set("platform", "discord");
  if (userId) url.searchParams.set("platform_user_id", String(userId));
  if (interaction?.guild_id) url.searchParams.set("guild_id", String(interaction.guild_id));
  url.searchParams.delete("dao");

  return url.toString();
}

async function fetchSpaceSaltOrThrow(connection, daoPk) {
  const gvr = await getGvr();
  const { deriveSpacePda } = gvr;
  const [spacePda] = deriveSpacePda(daoPk);
  const acct = await connection.getAccountInfo(spacePda, "confirmed");
  if (!acct) throw new Error(`Space not found: ${spacePda.toBase58()}`);
  const salt = parseSpaceSalt(acct.data);
  if (!salt || salt.length !== 32) throw new Error("Failed to parse space salt");
  return { spacePda, salt };
}

async function handlePing(res) {
  return json(res, 200, reply("✅ gv pong", { ephemeral: true }));
}

async function handleSetup(interaction, res) {
  const { opts } = optMap(interaction);
  const guildId = interaction?.guild_id ? String(interaction.guild_id) : "";
  const userId = interactionUserId(interaction);

  if (!guildId) {
    return json(res, 200, reply("❌ `/gv setup` can only be used in a server.", { ephemeral: true }));
  }
  if (!hasSetupPermission(interaction)) {
    return json(
      res,
      200,
      reply("❌ You need `Manage Server` (or Administrator) to run `/gv setup`.", { ephemeral: true })
    );
  }

  const daoPk = parsePubkeyOrThrow(opts.dao, "DAO");
  const roleId = opts.verified_role ? String(opts.verified_role) : null;
  if (roleId && !/^\d{17,22}$/.test(roleId)) {
    return json(res, 200, reply("❌ `verified_role` is invalid.", { ephemeral: true }));
  }

  const next = await setGuildConfig(guildId, {
    dao: daoPk.toBase58(),
    ...(roleId ? { verifiedRoleId: roleId } : {}),
    updatedBy: userId || "unknown",
  });

  const lines = [
    "✅ Guild verification config updated.",
    `Guild: \`${guildId}\``,
    `DAO: \`${next.dao}\``,
    next.verifiedRoleId
      ? `Verified role: <@&${next.verifiedRoleId}> (\`${next.verifiedRoleId}\`)`
      : "Verified role: not set (set `GVR_VERIFIED_ROLE_ID` or run setup with a role)",
  ];
  return json(res, 200, reply(lines.join("\n"), { ephemeral: true }));
}

async function handleVerify(interaction, res) {
  const dao = await resolvedDaoForInteraction(interaction);
  const url = buildVerificationUrl(interaction, dao);

  return json(
    res,
    200,
    replyWithLink(
      `Open the verification portal to complete your verification:\n<${url}>`,
      "Verify on Web",
      url,
      { ephemeral: true }
    )
  );
}

async function handleCheck(interaction, res) {
  const gvr = await getGvr();
  const { deriveSpacePda, identityHash, deriveIdentityPda } = gvr;

  const userId = interactionUserId(interaction);
  if (!userId) {
    return json(res, 200, reply("❌ Could not resolve your Discord user id from this interaction.", { ephemeral: true }));
  }

  const daoStr = await resolvedDaoForInteraction(interaction);
  if (!daoStr) {
    return json(
      res,
      200,
      reply(
        "❌ No DAO is configured for this server. Ask an admin to run `/gv setup` (or set `GVR_DAO` / `GVR_GUILD_DAO_MAP`).",
        { ephemeral: true }
      )
    );
  }

  const daoPk = parsePubkeyOrThrow(daoStr, "DAO");
  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });

  const [spacePda] = deriveSpacePda(daoPk);
  const spaceAcct = await connection.getAccountInfo(spacePda, "confirmed");
  const verifyLink = buildVerificationUrl(interaction, daoPk.toBase58());

  if (!spaceAcct) {
    return json(
      res,
      200,
      replyWithLink(
        `❌ DAO space is not initialized for \`${daoPk.toBase58()}\`.\nAsk an admin to run \`/gv admin space-init\`.`,
        "Open Verification",
        verifyLink,
        { ephemeral: true }
      )
    );
  }

  const salt = parseSpaceSalt(spaceAcct.data);
  if (!salt || salt.length !== 32) {
    return json(res, 200, reply("❌ Failed to parse space salt.", { ephemeral: true }));
  }

  const idh = identityHash(salt, platformTag("discord", gvr), String(userId));
  const [identityPda] = deriveIdentityPda(spacePda, platformSeed("discord", gvr), idh);
  const identityAcct = await connection.getAccountInfo(identityPda, "confirmed");

  if (!identityAcct) {
    const roleLine = await syncVerifiedRole(interaction, false);
    return json(
      res,
      200,
      replyWithLink(
        `❌ Not verified yet for DAO \`${daoPk.toBase58()}\`.\nDiscord user: \`${userId}\`${roleLine}`,
        "Verify on Web",
        verifyLink,
        { ephemeral: true }
      )
    );
  }

  const roleLine = await syncVerifiedRole(interaction, true);
  return json(
    res,
    200,
    reply(
      `✅ Verified for DAO \`${daoPk.toBase58()}\`.\nDiscord user: \`${userId}\`\nIdentity PDA: \`${identityPda.toBase58()}\`${roleLine}`,
      { ephemeral: true }
    )
  );
}

async function handleStatus(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, deriveSpacePda, identityHash, deriveIdentityPda, walletHash, deriveLinkPda } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();
  const walletStr = opts.wallet ? String(opts.wallet).trim() : "";

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });

  const [spacePda] = deriveSpacePda(daoPk);
  const spaceAcct = await connection.getAccountInfo(spacePda, "confirmed");

  let salt = null;
  if (spaceAcct) salt = parseSpaceSalt(spaceAcct.data);

  const idh = salt ? identityHash(salt, platformTag(platform, gvr), platformUserId) : null;

  const idSeed = platformSeed(platform, gvr);
  const identityPda = salt ? deriveIdentityPda(spacePda, idSeed, idh)[0] : null;

  let linkPda = null;
  if (walletStr && salt && identityPda) {
    const walletPk = new PublicKey(walletStr);
    const wh = walletHash(salt, walletPk);
    linkPda = deriveLinkPda(identityPda, wh)[0];
  }

  const lines = [
    `**Program:** \`${PROGRAM_ID.toBase58()}\``,
    `**DAO:** \`${daoPk.toBase58()}\``,
    `**Space PDA:** \`${spacePda.toBase58()}\`  ${spaceAcct ? "✅ exists" : "❌ missing"}`,
    salt ? `**Salt:** \`${Buffer.from(salt).toString("hex")}\`` : `**Salt:** —`,
    `**Platform:** \`${platform}\` (seed=${idSeed})`,
    `**Platform User ID:** \`${platformUserId}\``,
    idh ? `**id_hash:** \`${Buffer.from(idh).toString("hex")}\`` : `**id_hash:** —`,
    identityPda ? `**Identity PDA:** \`${identityPda.toBase58()}\`` : `**Identity PDA:** —`,
    linkPda ? `**Link PDA:** \`${linkPda.toBase58()}\`` : `**Link PDA:** —`,
  ];

  return json(res, 200, replyWithEmbed("Grape Verification Status", lines.join("\n"), { ephemeral: true }));
}

async function handleSpaceInit(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, buildInitializeSpaceIx } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const saltHex = String(opts.salt_hex || "").trim();
  if (!/^[0-9a-fA-F]{64}$/.test(saltHex)) {
    return json(res, 200, reply("❌ salt_hex must be 64 hex chars (32 bytes)", { ephemeral: true }));
  }

  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY"); // your bot wallet funds tx
  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });

  const salt = Uint8Array.from(Buffer.from(saltHex, "hex"));

  const { ix } = buildInitializeSpaceIx({
    daoId: daoPk,
    salt,
    authority: payer.publicKey,
    payer: payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);
  const sig = await sendTx(connection, tx, [payer]);

  return json(res, 200, reply(`✅ Space initialized.\nTx: \`${sig}\``, { ephemeral: true }));
}

async function handleAttest(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, identityHash, buildAttestIdentityIx } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();
  const expiresSeconds = opts.expires_seconds ? Number(opts.expires_seconds) : 0;

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });
  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY"); // also acts as attestor

  const { salt } = await fetchSpaceSaltOrThrow(connection, daoPk);

  const idh = identityHash(salt, platformTag(platform, gvr), platformUserId);
  const seed = platformSeed(platform, gvr);

  const now = Math.floor(Date.now() / 1000);
  const expiresAt = BigInt(expiresSeconds > 0 ? now + expiresSeconds : 0);

  const { ix } = buildAttestIdentityIx({
    daoId: daoPk,
    platform: platformEnum(platform, gvr),
    platformSeed: seed,
    idHash: idh,
    expiresAt,
    attestor: payer.publicKey,
    payer: payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);
  const sig = await sendTx(connection, tx, [payer]);

  return json(res, 200, reply(`✅ Identity attested.\nTx: \`${sig}\``, { ephemeral: true }));
}

async function handleRevoke(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, identityHash, buildRevokeIdentityIx } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });
  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY"); // also attestor

  const { salt } = await fetchSpaceSaltOrThrow(connection, daoPk);

  const idh = identityHash(salt, platformTag(platform, gvr), platformUserId);
  const seed = platformSeed(platform, gvr);

  const { ix } = buildRevokeIdentityIx({
    daoId: daoPk,
    platform: platformEnum(platform, gvr),
    platformSeed: seed,
    idHash: idh,
    attestor: payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);
  const sig = await sendTx(connection, tx, [payer]);

  return json(res, 200, reply(`✅ Identity revoked.\nTx: \`${sig}\``, { ephemeral: true }));
}

async function handleLink(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, identityHash, walletHash, buildLinkWalletIx } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();
  const walletPk = new PublicKey(String(opts.wallet));

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });
  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY");

  const { salt } = await fetchSpaceSaltOrThrow(connection, daoPk);

  const idh = identityHash(salt, platformTag(platform, gvr), platformUserId);
  const wh = walletHash(salt, walletPk);
  const seed = platformSeed(platform, gvr);

  const { ix } = buildLinkWalletIx({
    daoId: daoPk,
    platformSeed: seed,
    idHash: idh,
    wallet: walletPk,
    walletHash: wh,
    attestor: payer.publicKey,
    payer: payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);
  const sig = await sendTx(connection, tx, [payer]);

  return json(res, 200, reply(`✅ Wallet linked (attestor-driven).\nTx: \`${sig}\``, { ephemeral: true }));
}

async function handleLinkSelf(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, identityHash, walletHash, buildLinkWalletSelfIx } = gvr;
  // NOTE: This requires the *wallet* to be a signer on the transaction.
  // Discord cannot make a user's wallet sign. This is useful only when your "wallet"
  // is also a server-controlled keypair (e.g. a service wallet) OR you provide a client flow.
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();
  const walletStr = String(opts.wallet);

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });

  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY");
  const walletSigner = parseKeypairFromEnv("BOT_WALLET_SIGNER_SECRET_KEY"); // must match opts.wallet

  if (walletSigner.publicKey.toBase58() !== walletStr) {
    return json(
      res,
      200,
      reply(
        `❌ link-self requires BOT_WALLET_SIGNER_SECRET_KEY to match wallet argument.\n` +
          `wallet arg: ${walletStr}\n` +
          `wallet signer: ${walletSigner.publicKey.toBase58()}`,
        { ephemeral: true }
      )
    );
  }

  const { salt } = await fetchSpaceSaltOrThrow(connection, daoPk);

  const idh = identityHash(salt, platformTag(platform, gvr), platformUserId);
  const wh = walletHash(salt, walletSigner.publicKey);
  const seed = platformSeed(platform, gvr);

  const { ix } = buildLinkWalletSelfIx({
    daoId: daoPk,
    platformSeed: seed,
    idHash: idh,
    wallet: walletSigner.publicKey, // signer
    walletHash: wh,
    payer: payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);

  // both payer + wallet signer must sign
  const sig = await sendTx(connection, tx, [payer, walletSigner]);

  return json(res, 200, reply(`✅ Wallet self-linked.\nTx: \`${sig}\``, { ephemeral: true }));
}

async function handleUnlink(interaction, res) {
  const gvr = await getGvr();
  const { PROGRAM_ID, identityHash, walletHash, buildUnlinkWalletIx } = gvr;
  const { opts } = optMap(interaction);

  const daoPk = new PublicKey(String(opts.dao));
  const platform = String(opts.platform);
  const platformUserId = String(opts.platform_user_id).trim();
  const walletPk = new PublicKey(String(opts.wallet));
  const recipientPk = opts.recipient ? new PublicKey(String(opts.recipient)) : null;

  const connection = new Connection(rpcUrl(), { commitment: "confirmed" });
  const payer = parseKeypairFromEnv("BOT_FEE_PAYER_SECRET_KEY");

  const { salt } = await fetchSpaceSaltOrThrow(connection, daoPk);

  const idh = identityHash(salt, platformTag(platform, gvr), platformUserId);
  const wh = walletHash(salt, walletPk);
  const seed = platformSeed(platform, gvr);

  const { ix } = buildUnlinkWalletIx({
    daoId: daoPk,
    platformSeed: seed,
    idHash: idh,
    walletHash: wh,
    attestor: payer.publicKey,
    recipient: recipientPk || payer.publicKey,
    programId: PROGRAM_ID,
  });

  const tx = new Transaction().add(ix);
  const sig = await sendTx(connection, tx, [payer]);

  return json(res, 200, reply(`✅ Wallet unlinked.\nTx: \`${sig}\``, { ephemeral: true }));
}

/* -----------------------------
   Main entry: handle slash commands
------------------------------ */

async function handleSlashCommand(interaction, res) {
  try {
    const { sub } = optMap(interaction);

    if (!sub) return json(res, 200, reply("Unknown command", { ephemeral: true }));

    if (sub === "setup") return handleSetup(interaction, res);
    if (sub === "verify") return handleVerify(interaction, res);
    if (sub === "check") return handleCheck(interaction, res);
    if (sub === "ping") return handlePing(res);
    if (sub === "status") return handleStatus(interaction, res);
    if (sub === "space-init") return handleSpaceInit(interaction, res);
    if (sub === "attest") return handleAttest(interaction, res);
    if (sub === "revoke") return handleRevoke(interaction, res);
    if (sub === "link") return handleLink(interaction, res);
    if (sub === "link-self") return handleLinkSelf(interaction, res);
    if (sub === "unlink") return handleUnlink(interaction, res);

    return json(res, 200, reply("Unknown subcommand", { ephemeral: true }));
  } catch (e) {
    console.error("[slash] error:", e);
    return json(res, 200, reply(`❌ ${String(e?.message || e)}`, { ephemeral: true }));
  }
}

module.exports = {
  COMMANDS,
  registerCommands,
  handleSlashCommand,
};
