    ...
    options: [
      {
        type: 1,
        name: "verify",
        description: "Open the web verification flow",
        options: [{ type: 3, name: "dao", description: "DAO pubkey (optional)", required: false }],
      },
      {
        type: 1,
        name: "check",
        description: "Check if your Discord account is verified",
        options: [
          {
            type: 3,
            name: "dao",
            description: "DAO pubkey (optional if GVR_DAO is set)",
            required: false,
          },
          {
            type: 3,
            name: "wallet",
            description: "Wallet pubkey to check link status (optional)",
            required: false,
          },
        ],
      },
...
async function handleVerify(interaction, res) {
  ...
  const dao = opts.dao ? String(opts.dao).trim() : "";
...
async function handleCheck(interaction, res) {
  const gvr = await getGvr();
  const { deriveSpacePda, identityHash, deriveIdentityPda, walletHash, deriveLinkPda } = gvr;
  const { opts } = optMap(interaction);
  ...
  const daoStr = opts.dao ? String(opts.dao).trim() : defaultDaoPubkey();
  if (!daoStr) {
    return json(
      res,
      200,
      reply("❌ Missing dao. Pass `/gv check dao:<pubkey>` or set `GVR_DAO` in env.", { ephemeral: true })
    );
  }
  ...
  if (walletStr) {
    ...
    const linkAcct = await connection.getAccountInfo(linkPda, "confirmed");
    walletLine = `\nWallet link (\`${walletPk.toBase58()}\`): ${linkAcct ? "✅ linked" : "❌ not linked"}`;
  }
  ...
  return json(
    ...
    200,
    reply(
      `✅ Verified for DAO \`${daoPk.toBase58()}\`.\nDiscord user: \`${userId}\`\nIdentity PDA: \`${identityPda.toBase58()}\`${walletLine}`,
      { ephemeral: true }
    )
  );
}
