...
async function syncVerifiedRole(interaction, verified) {
  ...
  const roleId = verifiedRoleForInteraction(interaction);
...
async function handleVerify(interaction, res) {
  const dao = daoForInteraction(interaction);
...
async function handleCheck(interaction, res) {
  ...
  const daoStr = daoForInteraction(interaction);
  if (!daoStr) {
    return json(
      res,
      200,
      reply(
        "❌ No DAO is configured for this server. Ask an admin to set `GVR_DAO` or `GVR_GUILD_DAO_MAP`.",
        { ephemeral: true }
      )
    );
  }

  const daoPk = new PublicKey(daoStr);
